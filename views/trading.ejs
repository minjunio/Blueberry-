...
<html lang="en">
<head>
    <title>Binance Pro Terminal - Algo Enhanced</title>
    <meta charset="UTF-8">
    ...
    <style>
        ...
        /* Dramatic Asset Box Animations */
        .dramatic-box { transition: all 0.2s cubic-bezier(0.175, 0.885, 0.32, 1.275); position: relative; }
        .dramatic-box:hover { transform: scale(1.05) translateY(-4px); border-color: #F3BA2F; z-index: 10; box-shadow: 0 15px 30px rgba(0,0,0,0.9), inset 0 0 0 1px rgba(243,186,47,0.5); }
        ...
        /* Specific Asset Animations */
        .dot-spin { animation: spin 4s linear infinite; }
        .eth-pulse { animation: subtlePulse 2s ease-in-out infinite; }
        @keyframes spin { to { transform: rotate(360deg); } }
        @keyframes subtlePulse { 0%, 100% { transform: scale(1); filter: brightness(1); } 50% { transform: scale(1.1); filter: brightness(1.3); } }
    </style>
...
<body>
                    ...
                    <div class="panel p-3 md:p-4 flex flex-col gap-3 border-l-4 border-l-[#F3BA2F]">
                        <div class="flex justify-between items-center border-b border-[#2b3139] pb-2">
                            <div class="flex items-center gap-2">
                                ...
                                <h3 class="text-[10px] font-black uppercase tracking-widest text-[#F3BA2F]">Auto-Quant Engine (v7.0 Advanced Signals)</h3>
                            </div>
                            <div class="flex gap-2">
                                <button onclick="exportModel()" class="text-[9px] font-black uppercase text-muted hover:text-white px-2 py-0.5 border border-[#2b3139] rounded hover:border-white transition">Export Algo</button>
                                <button onclick="document.getElementById('import-model').click()" class="text-[9px] font-black uppercase text-muted hover:text-white px-2 py-0.5 border border-[#2b3139] rounded hover:border-white transition">Load Algo</button>
                                <input type="file" id="import-model" hidden accept=".js" onchange="importModel(event)">
                            ...
                            <div class="flex-1 grid grid-cols-2 gap-3">
                                <div class="bg-[#0b0e11] border border-[#2b3139] p-2 rounded flex flex-col justify-between group">
                                    <div class="flex justify-between items-center mb-1">
                                        <span class="text-[9px] text-muted font-black uppercase tracking-widest">Short-Term (Scalp)</span>
                                        <span id="ai-short-conf" class="text-[8px] font-mono text-[#F3BA2F]">0% Conf</span>
                                    </div>
                                    <div class="flex justify-between items-end mt-1">
                                        <span id="ai-short-signal" class="text-xs font-black uppercase text-white">ANALYZING...</span>
                                        <button id="ai-short-btn" onclick="executeAISuggestion('short')" class="px-2 py-1 bg-[#1e2329] border border-[#2b3139] group-hover:border-[#F3BA2F] text-[#F3BA2F] rounded text-[8px] font-black uppercase transition hidden">Trade</button>
                                    </div>
                                </div>
                                <div class="bg-[#0b0e11] border border-[#2b3139] p-2 rounded flex flex-col justify-between group">
                                    <div class="flex justify-between items-center mb-1">
                                        <span class="text-[9px] text-muted font-black uppercase tracking-widest">Long-Term (Swing)</span>
                                        <span id="ai-long-conf" class="text-[8px] font-mono text-[#F3BA2F]">0% Conf</span>
                                    </div>
                                    <div class="flex justify-between items-end mt-1">
                                        <span id="ai-long-signal" class="text-xs font-black uppercase text-white">ANALYZING...</span>
                                        <button id="ai-long-btn" onclick="executeAISuggestion('long')" class="px-2 py-1 bg-[#1e2329] border border-[#2b3139] group-hover:border-[#F3BA2F] text-[#F3BA2F] rounded text-[8px] font-black uppercase transition hidden">Trade</button>
                                    </div>
                                </div>
                            </div>

                            <div class="w-full md:w-64 bg-[#0b0e11] border border-[#2b3139] p-2 rounded flex flex-col justify-center">
                                <span class="text-[8px] uppercase font-black text-[#F3BA2F] mb-1">Hedge Paper Trading (Algo Status)</span>
                                <div class="flex justify-between items-center font-mono text-xs font-black">
                                    <span id="ai-paper-pnl" class="text-white">$0.00</span>
                                    <span id="ai-paper-winrate" class="text-muted">Win: 0% (0/0)</span>
                                </div>
                                <span id="ai-active-papers" class="text-[8px] text-muted mt-1 font-mono">0 active virtual trades hedging...</span>
                            </div>
                        ...
                        <div class="flex justify-between items-start z-20">
                            <div class="relative w-fit">
                                <button onclick="toggleDropdown('term-dropdown')" class="neo-input px-3 py-2 rounded flex items-center gap-3 w-[150px] md:w-48 bg-[#0b0e11]">
                                    <img id="term-logo" src="https://cryptologos.cc/logos/bitcoin-btc-logo.svg?v=029" class="w-5 h-5 rounded-full bg-[#1e2329] p-0.5">
                    ...
                    <div class="panel p-4 flex flex-col relative z-10">
                        ...
                        <div class="space-y-5">
                            ...
                            <div class="p-3 bg-[#0b0e11] border border-[#2b3139] rounded space-y-2">
                                ...
                                <div class="flex justify-between text-[9px] font-black uppercase border-t border-[#2b3139] pt-2">
                        ...
                        <button onclick="executeOrder()" id="btn-execute" class="mt-4 w-full py-3 rounded font-black uppercase tracking-[0.1em] text-[11px] bg-green-500 text-black hover:bg-green-400 transition">
                            Execute Long
                        </button>
                    </div>
        ...
        <main class="flex flex-col p-2 md:p-6 gap-6 w-full max-w-[1400px] mx-auto">
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-center md:items-end w-full">
                        <div class="flex flex-col items-center md:items-start">
                            ...
                            <div class="flex items-center gap-3 mt-2">
                                <span id="wallet-daily-pnl" class="text-xs font-black text-green-400 tracking-wide">+ 0.00</span>
                                <span id="wallet-daily-roi" class="text-[9px] font-black text-green-500 bg-[#0b0e11] px-1.5 py-0.5 rounded border border-[#2b3139]">+0.00% Today</span>
                            </div>
                        </div>

                        <div class="flex gap-3 mt-6 md:mt-0 w-full md:w-auto">
                            <button onclick="toggleModal('send-modal')" class="flex-1 md:flex-none px-6 py-2.5 rounded text-[10px] font-black uppercase tracking-widest bg-[#F3BA2F] text-black hover:bg-yellow-400 transition">Send / Withdraw</button>
                            <button onclick="toggleModal('swap-modal')" class="flex-1 md:flex-none px-6 py-2.5 rounded text-[10px] font-black uppercase tracking-widest bg-[#0b0e11] text-[#F3BA2F] border border-[#2b3139] hover:border-[#F3BA2F] transition">Swap Assets</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div>
                <div class="flex justify-between items-end mb-4 border-b border-[#2b3139] pb-2">
                    ...
                    <button onclick="toggleModal('settings-modal')" class="text-[9px] text-[#F3BA2F] hover:text-white uppercase font-black transition">Manage Portfolio</button>
                </div>
                <div id="wallet-square-grid" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3"></div>
            ...
            <div class="panel p-5">
                <div class="flex items-center gap-3 mb-4 pb-3 border-b border-[#2b3139]">
                    <div class="w-8 h-8 rounded bg-[#0b0e11] flex items-center justify-center border border-[#2b3139]">
                        <svg class="w-4 h-4 text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                    </div>
                    <h2 class="text-sm font-black uppercase text-white tracking-widest">Binance Earn</h2>
                </div>
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3" id="staking-grid"></div>
                <div class="mt-6">
                    <h3 class="text-[9px] font-black uppercase text-muted mb-3 tracking-widest border-b border-[#2b3139] pb-1">Active Stakes</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3" id="active-stakes-body"></div>
                </div>
            </div>
        </main>
    </div>
    ...
    <div id="ai-code-modal" class="fixed inset-0 z-[100] modal-overlay modal-hidden flex items-center justify-center p-4">
        <div class="modal-content panel w-full max-w-2xl p-5 relative flex flex-col">
            ...
            <p class="text-[9px] text-muted mb-4 font-mono">Vars: rsi, sma, volatility, prices[], buyRatio15m, buyRatio1h, whaleSentiment, topBuysAmount, topSellsAmount.<br>Return JSON. Use 'WAIT' to pause execution when signals do not align.</p>
    ...
    <div id="settings-modal" class="fixed inset-0 z-[100] modal-overlay modal-hidden flex items-center justify-center p-4">
        <div class="modal-content panel w-full max-w-sm p-5 relative border-t-2 border-t-[#F3BA2F]">
            ...
            <h2 class="text-xs font-black uppercase tracking-widest text-white mb-5 border-b border-[#2b3139] pb-2">Portfolio Management</h2>
            <div class="space-y-5">
                <div>
                    <label class="text-[9px] font-black uppercase tracking-widest text-[#F3BA2F] mb-1.5 block">Override USDT Balance</label>
                    <div class="flex gap-2"><input type="number" id="add-usdt-input" placeholder="Set Exact Amount" class="flex-grow neo-input p-2 rounded text-sm font-mono"><button onclick="setUsdt()" class="bg-[#F3BA2F] px-4 rounded text-[9px] font-black uppercase hover:bg-yellow-400 transition text-black">Set</button></div>
                </div>
                <div class="pt-4 border-t border-[#2b3139]">
                    <label class="text-[9px] font-black uppercase tracking-widest text-muted mb-1.5 block">Modify Specific Assets</label>
                    <div class="flex gap-2"><select id="new-asset-name" class="w-1/2 neo-input p-2 rounded text-xs font-black uppercase border border-[#2b3139]"></select><input type="number" id="new-asset-amt" placeholder="Qty (0 to Del)" class="w-1/2 neo-input p-2 rounded text-xs font-mono"></div>
                    <button onclick="addManualAsset()" class="w-full mt-3 neo-btn py-2.5 rounded text-[9px] font-black uppercase bg-[#2b3139] hover:bg-[#F3BA2F] hover:text-black text-white">Save Changes</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div id="send-modal" class="fixed inset-0 z-[100] modal-overlay modal-hidden flex items-center justify-center p-4">
        <div class="modal-content panel w-full max-w-md p-5 relative flex flex-col border-t-2 border-t-[#F3BA2F]">
            ...
            <h2 class="text-xs font-black uppercase tracking-widest text-white mb-5 border-b border-[#2b3139] pb-2">Send Crypto via API</h2>
            
            <div id="send-form" class="space-y-4">
                <div><label class="text-[9px] font-black uppercase tracking-widest text-muted block mb-1">Asset</label><select id="send-asset" class="w-full neo-input p-2.5 rounded text-xs font-black uppercase" onchange="updateSendMax()"></select></div>
                <div><label class="text-[9px] font-black uppercase tracking-widest text-muted block mb-1">Destination Address</label><input type="text" id="send-address" placeholder="0x..." class="w-full neo-input p-2.5 rounded text-xs font-mono"></div>
                <div>
                    <div class="flex justify-between text-xs mb-1"><label class="text-[9px] font-black uppercase tracking-widest text-muted">Amount</label><span class="text-[9px] font-black text-[#F3BA2F] cursor-pointer" onclick="setMaxSend()">MAX (<span id="send-avail-txt">0.00</span>)</span></div>
                    <input type="number" id="send-amount" placeholder="0.00" class="w-full neo-input p-2.5 rounded text-xs font-mono">
                </div>
                <button onclick="initiateSend()" class="mt-2 w-full py-3 rounded font-black uppercase tracking-[0.1em] text-[10px] bg-[#F3BA2F] text-black hover:bg-yellow-400">Broadcast TX</button>
            </div>

            <div id="send-processing" class="hidden flex-col items-center justify-center py-6">
                <div class="spinner w-8 h-8 mb-4"></div>
                <h3 class="text-[10px] font-black uppercase tracking-widest text-[#F3BA2F] mb-3" id="send-status-title">Broadcasting...</h3>
                <div class="w-full bg-[#0b0e11] p-3 rounded border border-[#2b3139] text-[9px] font-mono text-muted h-32 overflow-y-auto no-scrollbar" id="send-log"></div>
            </div>
        </div>
    </div>

    <div id="swap-modal" class="fixed inset-0 z-[100] modal-overlay modal-hidden flex items-center justify-center p-4">
        <div class="modal-content panel w-full max-w-sm p-5 relative flex flex-col">
            <button onclick="toggleModal('swap-modal')" class="absolute top-4 right-4 text-muted hover:text-white" id="swap-close-btn"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg></button>
            <h2 class="text-xs font-black uppercase tracking-widest text-white mb-5 border-b border-[#2b3139] pb-2">Swap Assets</h2>
            
            <div id="swap-form-content" class="space-y-3">
                <div class="bg-[#0b0e11] p-3 rounded border border-[#2b3139]">
                    <div class="flex justify-between text-[9px] font-black uppercase text-muted mb-2"><span>From</span><span id="swap-bal-from" class="text-white">Bal: 0.00</span></div>
                    <div class="flex gap-2"><input type="number" id="swap-amt-from" placeholder="0.00" class="flex-grow bg-transparent text-white text-lg font-mono outline-none w-full" oninput="calcSwap()"><select id="swap-asset-from" class="w-24 neo-input p-1.5 rounded text-xs font-black uppercase bg-[#1e2329]" onchange="updateSwapBals(); calcSwap()"></select></div>
                </div>
                <div class="flex justify-center -my-3 relative z-10"><div class="bg-[#2b3139] p-1.5 rounded border border-[#0b0e11]"><svg class="w-4 h-4 text-[#F3BA2F]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4"></path></svg></div></div>
                <div class="bg-[#0b0e11] p-3 rounded border border-[#2b3139]">
                    <div class="flex justify-between text-[9px] font-black uppercase text-muted mb-2"><span>To (Est.)</span><span id="swap-bal-to" class="text-[#F3BA2F]">Bal: 0.00</span></div>
                    <div class="flex gap-2"><input type="text" id="swap-amt-to" placeholder="0.00" class="flex-grow bg-transparent text-muted text-lg font-mono outline-none w-full" readonly><select id="swap-asset-to" class="w-24 neo-input p-1.5 rounded text-xs font-black uppercase bg-[#1e2329]" onchange="updateSwapBals(); calcSwap()"></select></div>
                </div>
                <div class="text-[9px] font-mono text-muted text-center py-1.5 rounded border border-[#2b3139] bg-[#0b0e11]" id="swap-rate-text">1 -- = -- --</div>
                <button onclick="executeSwap()" class="w-full py-3 mt-2 rounded font-black uppercase tracking-[0.1em] text-[10px] bg-[#F3BA2F] text-black hover:bg-yellow-400 transition">Confirm Swap</button>
            </div>
            <div id="swap-animation" class="hidden flex-col items-center justify-center py-10"></div>
        </div>
    </div>
    ...
    <div id="stake-modal" class="fixed inset-0 z-[100] modal-overlay modal-hidden flex items-center justify-center p-4">
        <div class="modal-content panel w-full max-w-sm p-5 relative flex flex-col">
            ...
            <h2 class="text-xs font-black uppercase tracking-widest text-white mb-2 border-b border-[#2b3139] pb-2">Stake <span id="stake-modal-asset" class="text-[#F3BA2F]">--</span></h2>
            <p class="text-[10px] text-green-400 font-mono mb-4">Est. APR: <span id="stake-modal-apr" class="font-black">--%</span></p>
            <div class="mb-4">
                <div class="flex justify-between text-[9px] font-black uppercase tracking-widest text-muted mb-1.5"><label>Amount to Stake</label><span class="text-[#F3BA2F] cursor-pointer" onclick="document.getElementById('stake-amount').value = document.getElementById('stake-modal-avail').dataset.val">MAX (<span id="stake-modal-avail" data-val="0">0.00</span>)</span></div>
                <input type="number" id="stake-amount" placeholder="0.00" class="w-full neo-input p-2.5 rounded text-sm font-mono">
            </div>
            <button onclick="confirmStake()" class="w-full py-3 rounded font-black uppercase tracking-[0.1em] text-[10px] bg-green-500 text-black hover:bg-green-400 transition">Lock & Stake</button>
        </div>
    </div>
    ...
    <script>
        ...
        const ASSETS = [
            { id: 'BTCUSDT', name: 'BTC/USDT', icon: 'bitcoin-btc-logo.svg?v=029', coin: 'BTC' }, { id: 'ETHUSDT', name: 'ETH/USDT', icon: 'ethereum-eth-logo.svg?v=029', coin: 'ETH' },
            { id: 'SOLUSDT', name: 'SOL/USDT', icon: 'solana-sol-logo.svg?v=029', coin: 'SOL' }, { id: 'BNBUSDT', name: 'BNB/USDT', icon: 'bnb-bnb-logo.svg?v=029', coin: 'BNB' },
            { id: 'XRPUSDT', name: 'XRP/USDT', icon: 'xrp-xrp-logo.svg?v=029', coin: 'XRP' }, { id: 'DOGEUSDT', name: 'DOGE/USDT', icon: 'dogecoin-doge-logo.svg?v=029', coin: 'DOGE' }, 
            { id: 'ADAUSDT', name: 'ADA/USDT', icon: 'cardano-ada-logo.svg?v=029', coin: 'ADA' }, { id: 'DOTUSDT', name: 'DOT/USDT', icon: 'polkadot-new-dot-logo.svg?v=029', coin: 'DOT' }, 
            { id: 'MATICUSDT', name: 'MATIC/USDT', icon: 'polygon-matic-logo.svg?v=029', coin: 'MATIC' }, { id: 'AVAXUSDT', name: 'AVAX/USDT', icon: 'avalanche-avax-logo.svg?v=029', coin: 'AVAX' },
            { id: 'ATOMUSDT', name: 'ATOM/USDT', icon: 'cosmos-atom-logo.svg?v=029', coin: 'ATOM' }
        ];

        const STAKING_OPTS = [
            { id: 'ATOM', apr: 14.5, tag: 'High Yield' }, { id: 'DOT', apr: 11.8, tag: 'Hot' }, { id: 'SOL', apr: 8.5, tag: 'Popular' }, 
            { id: 'AVAX', apr: 6.5, tag: 'Stable' }, { id: 'ETH', apr: 4.2, tag: 'Secure' }, { id: 'ADA', apr: 3.5, tag: 'Safe' }
        ];

        const FX_RATES = { USD: 1, AED: 3.67, EUR: 0.92, GBP: 0.79, CAD: 1.36, AUD: 1.52 };
        const FX_SYM = { USD: '$', AED: 'د.إ', EUR: '€', GBP: '£', CAD: 'C$', AUD: 'A$' };

        const defaultAlgoScript = `// Advanced Algorithmic Engine (Pure Quantitative Signals)
...
let macdHist = macdLine - macdPrev; // Momentum

// Calculate Bollinger Bands (20, 2)
let mean = sma;
let sumSq = prices.slice(-20).reduce((acc, val) => acc + Math.pow(val - mean, 2), 0);
let stdDev = Math.sqrt(sumSq / 20);
let upperBB = mean + (2 * stdDev); let lowerBB = mean - (2 * stdDev);
let currentPrice = prices[prices.length-1];

let flowImbalance = (buyRatio15m - 0.5) * 2; 
let whaleImbalance = (whaleSentiment - 0.5) * 2;

// 1. Short-Term Scalp Signals (RSI, MACD Momentum, Bollinger Bands, Flow)
let isShortBuy = rsi < 40 && macdHist > 0 && currentPrice < lowerBB * 1.01 && buyRatio15m > 0.55;
let isShortSell = rsi > 60 && macdHist < 0 && currentPrice > upperBB * 0.99 && buyRatio15m < 0.45;

if (isShortBuy && topBuysAmount > topSellsAmount) { 
    shortSide = 'buy'; shortConf = 82 + Math.round(flowImbalance*10); 
} else if (isShortSell && topSellsAmount > topBuysAmount) { 
    shortSide = 'sell'; shortConf = 80 - Math.round(flowImbalance*10); 
} else if (volatility > stdDev * 3 && Math.abs(flowImbalance) > 0.6) { 
    shortSide = flowImbalance > 0 ? 'buy' : 'sell'; shortConf = 75; 
}

// 2. Long-Term Swing Signals (Whales, MACD Trend, SMA Breakouts)
let isLongBuy = whaleSentiment > 0.6 && currentPrice > sma && macdLine > 0;
let isLongSell = whaleSentiment < 0.4 && currentPrice < sma && macdLine < 0;

if (isLongBuy) { 
    longSide = 'buy'; longConf = 88; 
} else if (isLongSell) { 
    longSide = 'sell'; longConf = 86; 
} else if (Math.abs(whaleImbalance) > 0.5) { 
    longSide = whaleImbalance > 0 ? 'buy' : 'sell'; longConf = 65; 
}

return JSON.stringify({ 
    short: { side: shortSide, lev: 50, conf: shortConf }, 
    long: { side: longSide, lev: 5, conf: longConf } 
});`;
        ...
        let appState = JSON.parse(localStorage.getItem('cloudquant_v10_algo')) || {
            activeWalletIdx: 0, currency: 'USD', aiLogic: defaultAlgoScript, globalTradeLogs: [], topHoldersSim: defaultHolders, lastRunTime: Date.now(),
            aiStats: { wins: 0, losses: 0, pnl: 0, activePaper: [] }, 
            wallets: [{ id: 1, name: "Main API", usdtBalance: 15000.00, startOfDayValue: 15000.00, orders: [], history: [], manualAssets: { 'BTC': 0.15, 'ETH': 3.0, 'DOT': 150, 'ATOM': 500 }, stakes: [] }]
        };
        ...
        function init() {
            ...
            connectMultiStream(); connectGlobalTradeStream(); triggerPairChange(); buildStakingGrid(); setInterval(updateWalletSystem, 1000);
            const missedTime = Date.now() - appState.lastRunTime; if(missedTime > 60000) setTimeout(() => { if(ui.price > 0) generateBackgroundData(ui.pair, ui.price, missedTime); }, 1500);
        }
        ...
        function refreshWorkspace() { renderWalletTabs(); buildNativeSelects(); renderOrders(); renderHistory(); updateWalletApp(); updateUI(); }

        // --- HELPERS ---
        function getIconPath(coin) { if (coin === 'USDT') return 'https://cryptologos.cc/logos/tether-usdt-logo.svg?v=029'; const a = ASSETS.find(x => x.id.replace('USDT','') === coin || x.coin === coin); if (a) return `https://cryptologos.cc/logos/${a.icon}`; return `https://ui-avatars.com/api/?name=${coin}&background=1e2329&color=F3BA2F&rounded=true`; }
        function getIconImg(coin, cls) { 
            let ex = coin === 'DOT' ? 'dot-spin' : (coin === 'ETH' ? 'eth-pulse' : '');
            return `<img src="${getIconPath(coin)}" class="${cls} ${ex} rounded bg-[#1e2329] p-0.5 border border-[#2b3139]">`; 
        }
        ...
        function toggleModal(id) { const el = document.getElementById(id); el.classList.toggle('modal-hidden'); el.classList.toggle('modal-active'); if(id === 'swap-modal' && el.classList.contains('modal-active')) prepSwapModal(); if(id === 'send-modal' && el.classList.contains('modal-active')) prepSendModal(); if(id === 'settings-modal' && el.classList.contains('modal-active')) buildNativeSelects(); }

        function buildTerminalDropdown() { document.getElementById('term-dropdown').innerHTML = ASSETS.map(a => `<div onclick="selectPair('${a.id}', '${a.name}', '${a.coin}')" class="px-3 py-2 hover:bg-[#2b3139] cursor-pointer flex items-center gap-2 transition border-b border-[#2b3139]"><img src="${getIconPath(a.coin)}" class="w-5 h-5 rounded bg-[#1e2329] p-0.5"><span class="text-[10px] font-black uppercase text-white tracking-widest">${a.name}</span></div>`).join(''); }
        ...
        function buildNativeSelects() {
            const sels = ['send-asset', 'swap-asset-from', 'swap-asset-to', 'new-asset-name']; let availOpts = `<option value="USDT">USDT</option>`;
            for(const c in w().manualAssets) { if(w().manualAssets[c] > 0) availOpts += `<option value="${c}">${c}</option>`; }
            let allOpts = `<option value="USDT">USDT</option>`; ASSETS.forEach(a => { if(a.coin !== 'USDT') allOpts += `<option value="${a.coin}">${a.coin}</option>`; });
            document.getElementById('send-asset').innerHTML = availOpts; document.getElementById('swap-asset-from').innerHTML = availOpts; document.getElementById('swap-asset-to').innerHTML = allOpts; document.getElementById('new-asset-name').innerHTML = allOpts;
        }
        ...
        function connectMultiStream() {
            ...
            w().stakes.forEach(s => { if (s.coin !== 'USDT') streamSet.add((s.coin+'USDT').toLowerCase() + '@ticker'); });
        ...
        function updateWalletSystem() {
            ...
            w().stakes.forEach(s => { let valUsd = s.amt * (livePrices[s.coin + 'USDT'] || 0); totalUsdValue += valUsd; });
            ...
            const wPnlEl = document.getElementById('wallet-daily-pnl'); const wRoiEl = document.getElementById('wallet-daily-roi');
            wPnlEl.innerText = `${dPnlLocal >= 0 ? '+' : ''}${sym}${dPnlLocal.toFixed(2)}`; wPnlEl.className = `text-xs font-black tracking-wide ${dPnlLocal >= 0 ? 'text-green-400' : 'text-pink-400'}`;
            wRoiEl.innerText = `${dPnlLocal >= 0 ? '+' : ''}${dRoi.toFixed(2)}% Today`; wRoiEl.className = `text-[9px] font-black px-1.5 py-0.5 rounded border bg-[#0b0e11] ${dPnlLocal >= 0 ? 'text-green-400 border-green-500/30' : 'text-pink-400 border-pink-500/30'}`;
        ...
        function drawChart() {
            ...
            if(canvasHoverPos !== null && canvasHoverPos <= gW) {
                ...
                chartCtx.beginPath(); chartCtx.moveTo(0, closest.y); chartCtx.lineTo(gW, closest.y); chartCtx.stroke(); chartCtx.setLineDash([]); chartCtx.beginPath(); chartCtx.arc(closest.x, closest.y, 4, 0, Math.PI*2); chartCtx.fillStyle = '#F3BA2F'; chartCtx.fill(); drawPriceTag(closest.y, closest.p);
            } else { const last = points[points.length-1]; chartCtx.beginPath(); chartCtx.arc(last.x, last.y, 4, 0, Math.PI*2); chartCtx.fillStyle = '#F3BA2F'; chartCtx.fill(); drawPriceTag(last.y, last.p); }
        }

        // --- ENHANCED ALGO ENGINE & HEDGE PAPER TRADING ---
        ...
        function processAIPaperTrades(livePrice) {
            ...
            // 1. Calculate Live ROI for all active trades
            for(let t of active) {
            ...
            // 2. Hedge Logic: Check Hedged Pairs (Find the winner > 3%, close the loser)
            for (let i = 0; i < active.length; i++) {
                let trade = active[i];
                if (trade.hedgeId && trade.currentRoi > 3) { 
                    let oppIdx = active.findIndex(t => t.hedgeId === trade.hedgeId && t.side !== trade.side);
                    if (oppIdx !== -1) {
                        let oppTrade = active[oppIdx];
                        appState.aiStats.losses++;
                        appState.aiStats.pnl += (oppTrade.margin * (oppTrade.currentRoi/100)); // Log the loss
                        active.splice(oppIdx, 1);
                        updated = true;
                        if(oppIdx < i) i--; // Adjust index since we shifted the array
                        delete trade.hedgeId; // Remove hedgeId so the winner rides solo to TP/SL
                    }
                }
            }

            // 3. Normal TP/SL logic for remaining trades
            ...
            if(updated) { saveState(); renderAIStats(); }
        }
        ...
        function calcAIEngine() {
            ...
            // 4. Top Alpha Data Injection
            let pairLogs = appState.globalTradeLogs.filter(l => l.pair === ui.pair && (now - l.time) <= ui.alphaTfMs);
            let topBuysAmount = pairLogs.filter(l => l.isBuy).sort((a,b) => b.usdVal - a.usdVal).slice(0,5).reduce((a,b)=>a+b.usdVal,0);
            let topSellsAmount = pairLogs.filter(l => !l.isBuy).sort((a,b) => b.usdVal - a.usdVal).slice(0,5).reduce((a,b)=>a+b.usdVal,0);

            // Update UI Data Top row
            ...
            try {
                ...
                // Render Short Term UI with WAIT state
                ...
                if (res.short.side === 'WAIT' || !res.short.side) {
                    sEl.innerText = 'WAITING FOR SETUP'; sEl.className = 'text-xs font-black uppercase text-muted';
                    sConf.innerText = '--'; sBtn.classList.add('hidden');
                } else {
                    sConf.innerText = res.short.conf + '% Conf'; sEl.innerText = `${res.short.lev}x ${res.short.side}`;
                    sEl.className = res.short.side === 'buy' ? 'text-xs font-black uppercase text-green-400' : 'text-xs font-black uppercase text-pink-400';
                    sBtn.classList.remove('hidden');
                }

                // Render Long Term UI with WAIT state
                ...
                if (res.long.side === 'WAIT' || !res.long.side) {
                    lEl.innerText = 'WAITING FOR SETUP'; lEl.className = 'text-xs font-black uppercase text-muted';
                    lConf.innerText = '--'; lBtn.classList.add('hidden');
                } else {
                    lConf.innerText = res.long.conf + '% Conf'; lEl.innerText = `${res.long.lev}x ${res.long.side}`;
                    lEl.className = res.long.side === 'buy' ? 'text-xs font-black uppercase text-green-400' : 'text-xs font-black uppercase text-pink-400';
                    lBtn.classList.remove('hidden');
                }

                // Autonomous HEDGING Trigger
                // If it isn't waiting, and hasn't maxed out active pairs, open a hedged position
                if(Math.random() > 0.8 && appState.aiStats.activePaper.length < 6 && res.short.side !== 'WAIT') {
                    ...
                    saveState(); renderAIStats();
                }

            } catch(e) { console.error("Algo Logic Error", e); }
        }
        ...
        function renderAIStats() {
            ...
            pnlEl.className = appState.aiStats.pnl >= 0 ? "text-green-400 drop-shadow" : "text-pink-400";
            document.getElementById('ai-paper-winrate').innerText = `Win: ${wr.toFixed(0)}% (${appState.aiStats.wins}/${total})`;
            document.getElementById('ai-active-papers').innerText = `${appState.aiStats.activePaper.length} active virtual trades hedging...`;
        }
        ...
        function executeAISuggestion(type) {
            ...
            if(!margin || margin <= 0 || margin > w().usdtBalance) return alert("Invalid margin or insufficient balance in active wallet.");
        ...
        function exportModel() { const blob = new Blob([appState.aiLogic], { type: 'text/javascript' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'AutoQuant_Algo_v7.js'; a.click(); URL.revokeObjectURL(url); }
        function importModel(e) { const file = e.target.files[0]; if(!file) return; const reader = new FileReader(); reader.onload = function(e) { appState.aiLogic = e.target.result; document.getElementById('ai-logic-editor').value = appState.aiLogic; saveState(); calcAIEngine(); alert("Algo Loaded Successfully"); }; reader.readAsText(file); }

        // --- GENERAL LOGIC & VISUALS ---
        ...
        function setLogFilter(f) {
            ui.logFilter = f;
            document.getElementById('filter-all').className = f==='all' ? "text-[9px] font-black uppercase bg-[#F3BA2F] text-black px-2 py-0.5 rounded" : "text-[9px] font-black uppercase text-muted hover:text-white px-2 py-0.5 rounded border border-transparent hover:border-[#F3BA2F]";
            document.getElementById('filter-whales').className = f==='whales' ? "text-[9px] font-black uppercase bg-[#F3BA2F] text-black px-2 py-0.5 rounded" : "text-[9px] font-black uppercase text-muted hover:text-white px-2 py-0.5 rounded border border-transparent hover:border-[#F3BA2F]";
            renderGlobalLogs();
        }
        ...
        function renderGlobalLogs() {
            ...
            feed.innerHTML = filtered.slice(0, 50).map(l => { 
                const cStr = l.isBuy ? 'text-green-400' : 'text-pink-400'; const bStr = l.isBuy ? 'border-green-500/30' : 'border-pink-500/30';
                return `<div class="flex justify-between items-center px-3 py-1.5 rounded bg-[#1e2329] border ${bStr} shrink-0"><span class="text-[9px] font-mono text-muted w-16">${l.addr}</span><span class="text-[9px] font-black ${cStr} uppercase">${l.pair.replace('USDT','')} ${l.isBuy?'BUY':'SELL'} ${l.qty.toFixed(l.price<1?0:4)}</span><span class="text-[9px] font-mono text-white">$${l.usdVal.toLocaleString(undefined, {maximumFractionDigits:0})}</span></div>`;
            }).join('');
        }
        ...
        function generateBackgroundData(symbol, currentPrice, timeGap) {
            ...
            for(let i=0; i<numTrades; i++) {
                ...
                let qty = usdVal / currentPrice; let time = now - (Math.random() * timeGap); appState.globalTradeLogs.push({ addr: '0x' + Math.random().toString(16).substring(2, 8).toUpperCase(), isBuy, qty, price: currentPrice, usdVal, pair: symbol, time });
            }
            appState.globalTradeLogs.sort((a,b) => b.time - a.time); if(appState.globalTradeLogs.length > 1500) appState.globalTradeLogs = appState.globalTradeLogs.slice(0, 1500);
            saveState(); renderGlobalLogs(); renderOnChainAlpha();
        }

        // --- EXECUTION LOGIC ---
        function setSide(side) {
            ui.side = side; document.getElementById('btn-buy').className = side==='buy' ? "flex-1 py-2.5 rounded text-[10px] font-black uppercase tracking-widest bg-[#1e2329] text-green-400 border border-green-500/50" : "flex-1 py-2.5 rounded text-[10px] font-black uppercase tracking-widest text-muted hover:text-green-400 border border-transparent";
            document.getElementById('btn-sell').className = side==='sell' ? "flex-1 py-2.5 rounded text-[10px] font-black uppercase tracking-widest bg-[#1e2329] text-pink-400 border border-pink-500/50" : "flex-1 py-2.5 rounded text-[10px] font-black uppercase tracking-widest text-muted hover:text-pink-400 border border-transparent";
        ...
        function renderOrders() {
            ...
            if (w().orders.length === 0) { container.innerHTML = `<div class="p-4 text-center text-muted font-black uppercase text-[9px] border border-[#2b3139] border-dashed rounded">No Active Trades in ${w().name}</div>`; return; }
            let html = '';
            w().orders.forEach(o => {
        ...
        function updateWalletApp() {
            ...
            addRow('USDT', w().usdtBalance, false); for (const [coin, amt] of Object.entries(w().manualAssets)) addRow(coin, amt, false); w().stakes.forEach(s => addRow(s.coin, s.amt, true)); rows.sort((a,b) => b.valUsd - a.valUsd);
            ...
            for(let i=0; i<renderCount; i++) {
                ...
                gridHtml += `<div class="dramatic-box panel p-4 flex flex-col items-center justify-center text-center gap-1.5 aspect-square cursor-pointer">${r.isStake ? '<span class="absolute top-1 right-1 px-1 py-0.5 rounded bg-[#1e2329] text-green-400 border border-green-500/30 text-[7px] uppercase font-black">Staked</span>' : ''}<div class="w-8 h-8 md:w-10 md:h-10 mb-1">${getIconImg(r.coin, 'w-full h-full')}</div><span class="font-black text-white text-xs md:text-sm uppercase tracking-widest">${r.coin}</span><span class="font-mono text-muted text-[9px] md:text-[10px]">${r.amt.toLocaleString(undefined, {maximumFractionDigits: 4})}</span><span class="font-mono text-white font-bold mt-auto bg-[#0b0e11] w-full py-1 rounded border border-[#2b3139] text-[10px] md:text-xs">${sym}${valLocal.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</span></div>`;
            }
            document.getElementById('wallet-square-grid').innerHTML = gridHtml;
            const btn = document.getElementById('show-more-btn'); if(rows.length > 6) { btn.classList.remove('hidden'); btn.innerText = isShowingAllAssets ? "Show Less" : "Show More Assets"; } else { btn.classList.add('hidden'); }

            let sBody = document.getElementById('active-stakes-body');
            if(w().stakes.length === 0) sBody.innerHTML = `<div class="col-span-full p-4 text-center text-muted text-[9px] uppercase font-black border border-[#2b3139] rounded border-dashed">No active stakes</div>`;
            else { sBody.innerHTML = w().stakes.map(s => { let d = (Date.now() - s.time) / 86400000; let earned = s.amt * (s.apr / 100) * (d / 365); return `<div class="card-row p-3 flex justify-between items-center group bg-[#1e2329]"><div class="flex items-center gap-2">${getIconImg(s.coin, 'w-6 h-6')}<div class="flex flex-col"><span class="text-white font-black text-[10px] uppercase">${s.coin}</span><span class="text-muted font-mono text-[8px]">${s.amt.toFixed(4)} Stkd</span></div></div><div class="flex flex-col text-right"><span class="text-green-400 font-black text-[10px]">${s.apr}% APR</span><span class="text-[#F3BA2F] font-mono text-[9px]">+${earned.toFixed(6)}</span></div></div>`; }).join(''); }
        }

        // --- ACTIONS ---
        function buildStakingGrid() { document.getElementById('staking-grid').innerHTML = STAKING_OPTS.map(opt => `<div class="dramatic-box panel p-3 flex flex-col items-center"><span class="text-[7px] bg-[#1e2329] text-green-400 px-1.5 py-0.5 rounded mb-2 font-black uppercase border border-green-500/30">${opt.tag}</span><div class="w-7 h-7 mb-1.5">${getIconImg(opt.id, 'w-full h-full')}</div><h3 class="text-xs font-black text-white mb-0.5 tracking-widest">${opt.id}</h3><span class="text-sm font-mono font-black text-green-400">${opt.apr}%</span><button class="w-full mt-2 py-1 rounded bg-[#0b0e11] border border-[#2b3139] text-[#F3BA2F] text-[8px] font-black uppercase hover:border-[#F3BA2F] transition" onclick="openStakeModal('${opt.id}', ${opt.apr})">Stake</button></div>`).join(''); }
        function openStakeModal(coin, apr) { document.getElementById('stake-modal-asset').innerText = coin; document.getElementById('stake-modal-apr').innerText = apr + '%'; let avail = coin === 'USDT' ? w().usdtBalance : (w().manualAssets[coin] || 0); document.getElementById('stake-modal-avail').innerText = avail.toFixed(4); document.getElementById('stake-modal-avail').dataset.val = avail; document.getElementById('stake-amount').value = ''; document.getElementById('stake-amount').dataset.coin = coin; document.getElementById('stake-amount').dataset.apr = apr; toggleModal('stake-modal'); }
        function confirmStake() { const el = document.getElementById('stake-amount'); const amt = parseFloat(el.value); const coin = el.dataset.coin; if(isNaN(amt) || amt <= 0 || amt > parseFloat(document.getElementById('stake-modal-avail').dataset.val)) return alert("Invalid amount."); if(coin === 'USDT') w().usdtBalance -= amt; else w().manualAssets[coin] -= amt; w().stakes.push({ coin, amt, apr: parseFloat(el.dataset.apr), time: Date.now() }); saveState(); toggleModal('stake-modal'); updateWalletApp(); updateWalletSystem(); }

        function prepSwapModal() { buildNativeSelects(); document.getElementById('swap-asset-from').value = 'USDT'; document.getElementById('swap-asset-to').value = 'BTC'; document.getElementById('swap-form-content').classList.remove('hidden'); document.getElementById('swap-animation').classList.add('hidden'); document.getElementById('swap-close-btn').classList.remove('hidden'); updateSwapBals(); calcSwap(); }
        function updateSwapBals() { let from = document.getElementById('swap-asset-from').value; let to = document.getElementById('swap-asset-to').value; document.getElementById('swap-bal-from').innerText = `Bal: ${(from === 'USDT' ? w().usdtBalance : (w().manualAssets[from]||0)).toFixed(4)}`; document.getElementById('swap-bal-to').innerText = `Bal: ${(to === 'USDT' ? w().usdtBalance : (w().manualAssets[to]||0)).toFixed(4)}`; }
        function calcSwap() { let from = document.getElementById('swap-asset-from').value; let to = document.getElementById('swap-asset-to').value; let amt = parseFloat(document.getElementById('swap-amt-from').value) || 0; if(from === to) { document.getElementById('swap-amt-to').value = amt; document.getElementById('swap-rate-text').innerText = `1 ${from} = 1 ${to}`; return; } let priceFromUsd = from === 'USDT' ? 1 : (livePrices[from+'USDT'] || 1); let priceToUsd = to === 'USDT' ? 1 : (livePrices[to+'USDT'] || 1); let toAmt = (amt * priceFromUsd) / priceToUsd; document.getElementById('swap-amt-to').value = toAmt.toFixed(6); document.getElementById('swap-rate-text').innerText = `1 ${from} ≈ ${(priceFromUsd/priceToUsd).toFixed(6)} ${to}`; }
        function executeSwap() { let from = document.getElementById('swap-asset-from').value; let to = document.getElementById('swap-asset-to').value; let amt = parseFloat(document.getElementById('swap-amt-from').value) || 0; let toAmt = parseFloat(document.getElementById('swap-amt-to').value) || 0; if(amt <= 0) return; let avail = from === 'USDT' ? w().usdtBalance : (w().manualAssets[from]||0); if(amt > avail) return alert("Insufficient balance in active wallet."); document.getElementById('swap-form-content').classList.add('hidden'); document.getElementById('swap-close-btn').classList.add('hidden'); const anim = document.getElementById('swap-animation'); anim.classList.remove('hidden'); anim.classList.add('flex'); anim.innerHTML = `<div class="spinner w-8 h-8 mb-4"></div><p class="text-[#F3BA2F] font-black uppercase text-[10px]">Swapping on-chain...</p>`; setTimeout(() => { anim.innerHTML = `<svg class="w-10 h-10 text-green-500 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg><p class="text-green-400 font-black uppercase text-[10px]">Swap Successful</p>`; if(from === 'USDT') w().usdtBalance -= amt; else w().manualAssets[from] -= amt; if(to === 'USDT') w().usdtBalance += toAmt; else { if(!w().manualAssets[to]) w().manualAssets[to] = 0; w().manualAssets[to] += toAmt; } connectMultiStream(); saveState(); setTimeout(() => { toggleModal('swap-modal'); updateWalletApp(); updateWalletSystem(); }, 1200); }, 1000); }

        function prepSendModal() { buildNativeSelects(); document.getElementById('send-asset').value = 'USDT'; updateSendMax(); document.getElementById('send-form').classList.remove('hidden'); document.getElementById('send-processing').classList.add('hidden'); document.getElementById('send-close-btn').classList.remove('hidden'); }
        function updateSendMax() { let coin = document.getElementById('send-asset').value; let avail = coin === 'USDT' ? w().usdtBalance : (w().manualAssets[coin]||0); document.getElementById('send-avail-txt').innerText = avail.toFixed(4); }
        function setMaxSend() { document.getElementById('send-amount').value = document.getElementById('send-avail-txt').innerText; }
        let sendInterval, sendTimeout; function initiateSend() { let asset = document.getElementById('send-asset').value; let addr = document.getElementById('send-address').value; let amt = parseFloat(document.getElementById('send-amount').value); if(!addr || addr.length < 10) return alert("Invalid address."); if(isNaN(amt) || amt <= 0 || amt > parseFloat(document.getElementById('send-avail-txt').innerText)) return alert("Invalid amount."); document.getElementById('send-form').classList.add('hidden'); document.getElementById('send-processing').classList.remove('hidden'); document.getElementById('send-processing').classList.add('flex'); document.getElementById('send-close-btn').classList.add('hidden'); const logBox = document.getElementById('send-log'); logBox.innerHTML = ''; const addLog = (msg, delay) => new Promise(res => { sendTimeout = setTimeout(() => { logBox.innerHTML += `<div>${msg}</div>`; logBox.scrollTop = logBox.scrollHeight; res(); }, delay); }); document.getElementById('send-status-title').innerText = "Initiating..."; (async function runFakeTx() { await addLog(`[SYS] Contract call ${amt} ${asset}...`, 300); await addLog(`[NET] Secure API RPC...`, 600); document.getElementById('send-status-title').innerText = "Awaiting Inclusion"; await addLog(`[MEM] TX accepted. Broadcasting...`, 1200); await addLog(`[VAL] Checking consensus...`, 1800); document.getElementById('send-status-title').innerText = "Verification Pending"; document.getElementById('send-status-title').classList.replace('text-[#F3BA2F]', 'text-pink-400'); await addLog(`<span class="text-pink-400 font-bold">[ERR] Validation stalled. Node mismatch.</span>`, 1800); document.getElementById('send-close-btn').classList.remove('hidden'); sendInterval = setInterval(() => { logBox.innerHTML += `<div class="text-muted">[VAL] Retrying...</div>`; logBox.scrollTop = logBox.scrollHeight; }, 3000); })(); }
        ...
        function addManualAsset() { const n = document.getElementById('new-asset-name').value.toUpperCase(); const a = parseFloat(document.getElementById('new-asset-amt').value); if(n) { if (a <= 0 || isNaN(a)) { delete w().manualAssets[n]; } else { w().manualAssets[n] = a; } document.getElementById('new-asset-name').value=''; document.getElementById('new-asset-amt').value=''; saveState(); connectMultiStream(); toggleModal('settings-modal'); updateWalletSystem(); updateWalletApp(); } }
        ...
        function saveState() { localStorage.setItem('cloudquant_v10_algo', JSON.stringify(appState)); updateUI(); }
